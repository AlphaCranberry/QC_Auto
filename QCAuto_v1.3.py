...
parser=argparse.ArgumentParser(prog = 'QC_Automation',
                               description = 'Auttomated QC function. this code will take files in either the current directory, or designated directory, and analyze all fastq.gz for QC analysis',
                               epilog = "Lets hope this don't break"
                               )
...
parser.add_argument("-i",
                    "--hisatGenome",
                    help = "specify genome to use for Hisat2 alinging. Format should be some/dir/prefix where some dir is the directory of the hisat index and prefix is the hisat built genome prefix")
parser.add_argument("-s",
                    "--starGenome",
                    help = "specify genome to use for Star alinging. Format should be some/dir/STARindex/ or something... idk this piece isn't finished yet.")


args=parser.parse_args()
    
# pull from parser
hisatGenome = args.hisatGenome
starGenome = args.starGenome
rawPath = args.raw
...
if (hisatGenome == None) and (starGenome == None):
...
## check validity of genome directories...
#starting with Hisat
if hisatGenome != None:
    useHisat = True

    hisatName = hisatGenome.split('/')[-1]
    hisatPath = hisatGenome.split(hisatName)[0]
    if not os.path.exists(hisatPath):
...
if starGenome != None:
    ...
    if starGenome[-1] == '/':
        starName = starGenome.split('/')[-2]
        starPath = starGenome.split(starName)[0]
        
        if not os.path.exists(starPath):
            exit(f'\nThe directory:\n{starPath}\nis not a known directory')

    if starGenome[-1] != '/':
        starName = starGenome.split('/')[-1]
        starPath = starGenome.split(starName)[0]
...
while maybe == True:
    ...
    if os.path.exists(parentDir):
        ...
        exit(f'{parentDir} does not exist')
    ...
    if buildTagDir == True: 
        ...
        for eachDir in os.listdir(rawPath):
            ...
            if useHisat == True:
                ...
                for file in os.listdir():
                    if file[-8:] == '.trimmed':
                        ...
                        print(f'Begining Hisat2 alinging {file}...')
                        filePath =f'{parentDir}/raw/{eachDir}'
                        hisatPro = multiprocessing.Process(target = hisat2Func, args = (file,hisatGenome,filePath,), name = "hisatPro_"+file.split('.')[0])
                        hisatPro.start()
                        #add to list to join later
            ...
            if useStar == True:
                ...
                for file in os.listdir():
                    if file[-8:] == '.trimmed':
                        ## run the subprocess with the function above. p.s. don't run more than 10 at once
                        print(f'Begining STAR alinging {file}...')
                        filePath =f'{parentDir}/raw/{eachDir}'
                        starPro = multiprocessing.Process(target = starFunc, args = (starGenome,file,), name = "starPro_"+file.split('.')[0])
                        starPro.start()
                        #add to list to join later
        ...
        cmdBatch = f'batchMakeTagDirectory.pl QC/sampleInfoFile.txt -cpu 8 -genome {homerGenome}  -omitSN -checkGC -fragLength 150 -single'
        os.system(cmdBatch)
    else:
        ...
        with open(f'{parentDir}/QC/sampleInfoFile.txt','w') as namesFile:
            for eachVal in samLst:
                ...
                if ('csInp' in eachVal) or ('_sRNA' in eachVal):
                    ## same here
                    if '_L0' in eachVal:
                        modVal = eachVal.split('_L0')[0]
                    else:
    ...
    with open(f'{parentDir}/QC/mappingStats.txt', 'w') as statstxt:
        statstxt.write("Samp Name\tUnpaired_Reads\tUnaligned\tUnique_Reads\tAligned_Multiple\tOverall_Alignment Rate\ttotal_#_reads\ttotal_#_unpaired\ttotal_#_unaligned\ttotal_#_unique\ttotal_#_aligned_multiple\n")
        for eachDir in os.listdir(rawPath):
            if os.path.isdir(f'{rawPath}/{eachDir}'):
                os.chdir(f'{rawPath}/{eachDir}')
    ...
    # begin pulling from the tagdirs mapping stats
    #start by moving to appropriate dirctory:
    os.chdir(f'{parentDir}/QC')

    with open ('tagDirMappingStats.txt', 'w') as tdMappingStats:
        # write the initial title line to the file
        columns = f'file\tuniquePositions\ttotalTags\ttagsPerBP\taverageTagsPerPosition\taverageFragmentGCcontent\n'
        print(columns)
        tdMappingStats.write(columns)

        with open('sampleInfoFile.txt', 'r') as sampInfo:
            for eachLine in sampInfo:
                tagDir = eachLine.split('\t')[0]
                tagDirStats = tagDir + '/tagInfo.txt'

                #start writing this information to a dataframe like object
                appLine = ''
                with open(tagDirStats, 'r') as tagStats:
                    fileName = tagDirStats.split('/')[-2]
                    appLine+= fileName+'\t'

                    for statLine in tagStats:
                        if 'genome=' in statLine:
                            appLine += statLine.split('\t')[1] + '\t'
                            appLine += statLine.split('\t')[2].strip() + '\t'
                        elif 'tagsPerBP' in statLine:
                            appVal = statLine.split('=')[-1].strip()
                            appLine += appVal + '\t'
                        elif 'averageTagsPerPosition' in statLine:
                            appVal = statLine.split('=')[-1].strip()
                            appLine += appVal + '\t'
                        elif 'averageFragmentGCcontent' in statLine:
                            appVal = statLine.split('=')[-1].strip()
                            appLine += appVal + '\t'

                #combine the string and write to the file
                print(appLine)
                appLine += '\n'
                tdMappingStats.write(appLine)
    ...
    #### below not currently being used #####
    ...
    sns.set(font_scale=1.4)
    g = sns.FacetGrid(combined_frames_stacked, col='Library', height=8, aspect=1, col_wrap=4)
